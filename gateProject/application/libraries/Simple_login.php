 ...
 class Simple_login {
     ...
     */
     public function login($username, $password) {
         ...
         $query = $this->CI->db->get_where('tbladmin',array('user'=>$username,'password' => $password));
 
         if($query->num_rows() == 1) {
             //ambil data user berdasar username
             $row  = $this->CI->db->query('SELECT id FROM tbladmin where user = "'.$username.'"');
             $admin     = $row->row();
             $id   = $admin->id;
 
             //set session user
             $this->CI->session->set_userdata('user', $username);
